...
Welcome to AutoServe! This guide explains how to quickly demonstrate and evaluate the AutoServe platform across all four user roles.

## Portfolio Access Credentials & Quick Hints

When logging in at `http://localhost` (or `http://localhost:5173` during local dev), select the target role card on the login screen. You can enter any valid email format with the role's persistent portfolio password:
...
| **Service Manager** | Service Manager Card | `manager0521` | Claim Pending Queue, Approve Appointments, Assign Mechanics, Add Parts, Invoice Generation |
...
4. Click **Pay Invoice** to complete payment.
