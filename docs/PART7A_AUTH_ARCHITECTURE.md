...
2. **Opaque Refresh Tokens**: 256-bit cryptographically secure random entropy strings generated via `SecureRandom`. Raw refresh tokens are never persisted in the database; only their 64-character SHA-256 hexadecimal text hashes are stored in `refresh_tokens.token_hash`.
